    ...
    return (
      <button
        ...
        className={cn(
          ...
          "focus-visible:ring-2 focus-visible:ring-heat-100 focus-visible:ring-offset-1",
          isActive
            ? "font-semibold text-accent-white"
            : "text-black-alpha-72 hover:text-accent-black",
      ...
      >
        {isActive && (
          <motion.span
            ...
            className="absolute inset-0 rounded-full bg-accent-black"
            transition={{
              type: "spring",
              stiffness: 400,
              damping: 30,
        ...
        <span className="relative z-10 flex items-center gap-2">
          {Icon && (
            <Icon
              className={cn(
                ...
                isActive ? "text-accent-white" : "text-black-alpha-48"
              )}
            />
          )}
          ...
          {badge && (
            <span className="rounded-full bg-heat-100 px-1.5 py-0.5 text-[10px] font-bold tracking-tight uppercase text-accent-white">
              {badge}
            </span>
          )}
        </span>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "focus-visible:ring-2 focus-visible:ring-heat-100",
        isSelected
          ? "bg-accent-black font-semibold text-accent-white"
    ...
    >
      <div className="flex items-center gap-2.5">
        {Icon && (
          <Icon
            className={cn(
              "size-4 shrink-0",
              isSelected ? "text-accent-white" : "text-black-alpha-48"
            )}
          />
        )}
      ...
      {isSelected && <Check className="size-3.5 text-accent-white" />}
    </button>
  );
}
...
export function NotchNav({
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 h-screen w-screen overflow-hidden p-0 md:p-2 transition-colors duration-200",
        className
      )}
      {...props}
    >
      <div className="relative flex h-full w-full flex-col rounded-none md:rounded-2xl bg-background-base text-accent-black antialiased transition-colors duration-200">
        <div
          aria-hidden="true"
          ...
          className={cn(
            ...
            isDropdownOpen
              ? "pointer-events-auto bg-black-alpha-40 backdrop-blur-[2px] opacity-100"
        ...
        {showLogo && logo && (
          <aside
            ...
            className={cn(
              "hidden xl:flex absolute left-0 z-50 h-10 px-5 select-none transition-colors duration-200 bg-background-base",
              isBottom
                ? "bottom-0 rounded-tr-[24px] md:items-end"
                : "top-0 rounded-br-[24px] md:items-baseline"
            )}
          >
            <div className="flex items-center text-accent-black">
              {logo}
            </div>
        ...
        <header
          ...
          className={cn(
            "hidden xl:flex absolute left-1/2 -translate-x-1/2 z-50 h-11 px-4 bg-background-base text-accent-black select-none transition-colors duration-200",
            isBottom
        ...
        {showRightContent && rightContent && (
          <aside
            ...
            className={cn(
              "hidden xl:flex absolute right-0 z-50 h-10 px-5 select-none transition-colors duration-200 bg-background-base",
              isBottom
        ...
        <div
          ref={containerRef}
          className={cn(
            "xl:hidden absolute z-50 flex flex-col bg-background-base text-accent-black select-none transition-colors duration-200",
        ...
        >
          ...
          >
            {/* Left Logo Slot */}
            {showLogo && logo && (
              <div className="flex shrink-0 items-center text-accent-black">
                {logo}
              </div>
            )}
            ...
            <button
              ...
              className="group flex h-8 sm:h-8.5 w-full cursor-pointer items-center justify-center gap-1.5 rounded-full px-2.5 py-2.5 sm:p-2.5 text-xs sm:text-sm font-semibold text-accent-black outline-none transition-colors sm:hover:bg-black-alpha-4 focus-visible:ring-2 focus-visible:ring-heat-100"
            >
              {activeItem?.icon && (
                <activeItem.icon className="size-3.5 sm:size-4 shrink-0 text-black-alpha-48" />
              )}
              ...
              {isBottom ? (
                <CaretUp
                  className={cn(
                    "size-3.5 text-black-alpha-48 transition-transform duration-200",
            ...
            {/* Right Action Slot */}
            {showRightContent && rightContent && (
              <div className="flex shrink-0 items-center justify-end text-accent-black w-max">
                {rightContent}
              </div>
            )}
          </div>

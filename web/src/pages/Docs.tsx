      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-0 px-4 md:grid-cols-[240px_1fr_220px]">
                    ...
                    <li key={it.id}>
                      <a
                        ...
                        className={cn(
                          ...
                          active === it.id ? "bg-brand/10 text-brand" : "text-fg-secondary hover:text-fg-primary",
                        )}
                      >
                        {it.title}
                      </a>
        ...
        <article className="min-w-0 max-w-3xl py-10 px-2 md:px-8">
          <div className="flex items-center justify-between">
            <Badge tone="brand">CLI & Developer Tooling</Badge>
            <div className="flex items-center gap-2">
              <button onClick={copyMd} className="inline-flex items-center gap-1 rounded-md border border-border bg-bg-elevated px-2 py-1 text-xs text-fg-secondary hover:text-brand" aria-label="Copy page as markdown">
          ...
          <h1 className="mt-4 font-display text-4xl font-semibold tracking-tight text-fg-primary">policyctl Documentation</h1>
          ...
          <p className="mt-2 text-fg-secondary">One <code className="font-mono">.policyctl.yml</code> declares rules. Hook-time and CI-time share the same engine.</p>
          <CodeBlock
          ...
          <div className="mt-3 overflow-hidden rounded-lg border border-border">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="border-b border-border/60"><td className="p-3 font-mono text-fg-primary">regex</td><td className="p-3 text-fg-secondary">Match diff content (e.g. secrets)</td></tr>
                <tr><td className="p-3 font-mono text-fg-primary">tool</td><td className="p-3 text-fg-secondary">Match a tool call (Bash, Edit, Write)</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <p className="mt-2 text-fg-secondary"><Badge tone="brand">hook</Badge> enforces at agent tool-call time; <Badge tone="accent">ci</Badge> enforces in the pipeline; <Badge tone="muted">both</Badge> enforces everywhere.</p>
          ...
          <div className="mt-10 flex items-center justify-between border-t border-border pt-6 text-sm">
            <a href={`${GITHUB}/edit/main/site/docs.html`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-fg-muted hover:text-brand"><GitHubIcon className="size-4" /> Edit this page on GitHub</a>
                ...
                <li key={it.id}>
                  <a
                    ...
                    className={cn(
                      ...
                      active === it.id ? "border-brand text-brand" : "border-transparent text-fg-muted hover:text-fg-primary",
                    )}
                  >
                    {it.title}
                  </a>

  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      <div className="mx-auto max-w-2xl px-6 py-16">
        <div className="font-mono text-xs uppercase tracking-[0.16em] text-brand">Welcome — let's get you set up</div>
        <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight text-fg-primary">Your first policy in four steps</h1>
        ...
        <ol className="mt-8 space-y-4">
          {steps.map((s, i) => (
            <li
              key={i}
              className={`rounded-xl border p-5 transition-colors ${done[i] ? "border-brand/40 bg-brand/5" : i === current ? "border-border bg-bg-surface" : "border-border/50 bg-bg-surface/50"}`}
            >
              <div className="flex items-center gap-3">
                <span className={`flex size-7 items-center justify-center rounded-pill font-mono text-xs ${done[i] ? "bg-brand text-bg-primary" : "bg-bg-elevated text-fg-secondary"}`}>
